...
resource "aws_iam_policy" "lambda_runner" {
  name = "${aws_iam_role.lambda_runner.name}-policy"
...
resource "aws_iam_role_policy_attachment" "lambda_runner" {
  role       = aws_iam_role.lambda_runner.name
  policy_arn = aws_iam_policy.lambda_runner.arn
}
